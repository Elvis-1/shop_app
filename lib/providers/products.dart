...
Products(this.authToken,this.userId, this._items);
  ...
  Future<void> fetchAndSetProducts([bool filterByUser=false]) async{
    ...
    try{
      ...
      // this was an empty list but i had to add one product just to have something to load due to poor internet connection
      ];
  ...
  Future<void> addProduct(Product product) async {
    ...
    try {
      ...
      // _items.insert(0, newProduct); // alternatively, at the start of the list
      notifyListeners();
      //  }).catchError((error){
      //    print(error);
      //    throw error; // this will create a new error and make it available in the other screen where it is called
      // });
    }catch(error){
      print(error);
      throw error;
    ...
    Future<void> deleteProduct(String id) async{
      ...
      final response = await http.delete(Uri.parse(url));
            ...
            if(response.statusCode>= 400){
              _items.insert(existingProductIndex, existingProduct);  // this will help to return the product back to list in case there is failure
              notifyListeners();
              throw HttpException('Could not delete product');
              // we are returning our error because delete does not return error
            }

            // existingProduct = null; // I had issues assingning this to null
      }
